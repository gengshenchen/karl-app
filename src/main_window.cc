...
MainWindow::MainWindow(QWidget *parent, const Qt::WindowFlags flags)
    ...
    //MainWindow 有自己的布局，要自定义布局需用widget类
...
void MainWindow::initialize() {
    ...
    // 此时的titleBarLayout已经有控件在布局里。所以addSpacing会在最后面添加，要使用insertSpacing。图标和文本TitleLabel 不属于布局。
    // 源码： titleBarLayout->addStretch(); //也是布局里的，index也要算
    //       titleBarLayout->addLayout(systemButtonsOuterLayout);//放大缩小关闭 一个布局
